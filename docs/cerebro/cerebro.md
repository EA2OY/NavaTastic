...
- **2026-08-25 — CORRECCIÓN DE MIGRACIÓN BINARIA EN `/resilience.bin` (L38) Y AUDITORÍA V5 EN HARDWARE REAL**:
  - **Fallo de Nombres Descubierto**: Al flashear la versión V5 sobre nodos que tenían un `/resilience.bin` antiguo de V4 (más corto, ~560B vs 756B), la falta de `memset` previo a `f.read()` dejaba basura en `custom_long_name`. El firmware creía que había un nombre fijado y sobreescribía `owner.long_name` con caracteres no imprimibles.
  - **Solución Implementada**:
    1. `memset(&prefs, 0, sizeof(prefs))` antes de leer el archivo de flash.
    2. `memset` forzado de `custom_long_name` y `custom_short_name` en la rama `if (legacy)`.
    3. Rutina de validación y auto-sanación de cadenas que detecta y purga cualquier residuo no imprimible, preservando siempre el nombre legítimo de `NodeDB`.
  - **Compilación 12/12 SUCCESS**: Toda la matriz de 12 entornos unificados compilada y distribuida a `distribucion\` y `Desktop\NavaTastic V5 4.3.4\`.
  - **Despliegue de Banco Físico V5**: Cadena lineal forzada de 4 nodos (`MasterNode` $\longleftrightarrow$ `GPS` $\longleftrightarrow$ `015a` $\longleftrightarrow$ `Slave`) a `-5 dBm / SFNarrow` lista y documentada en `docs/cerebro/12_auditoria_navatastic.md`. EngendroNode (E22P+BME680) verificado pero excluido de la cadena lineal por penetración RF (30 dBm).

### Errores encontrados → solución (consolidado, 2026-08-25)
...
| 14 | Corrupción de nombres al actualizar de V4 a V5 por memoria residual no inicializada en `/resilience.bin` (<756B) | L38: `memset` previo a `f.read()`, inicialización en `legacy` y auto-sanación de cadenas en `NavaCLIModule.cpp` | ✅ aplicado + verificado HW |
