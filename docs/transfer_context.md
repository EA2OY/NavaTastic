...
*   **Ronda 2026-08-15 (V2.6 — ciclo sueño/despertar definitivo, verificado en banco)**:
  - **Comportamiento final** (detalle y referencia histórica Eclipse V1 en `cerebro/04_energia_bateria.md`, sección nueva): [Sueño] tras 5 lecturas reales del monitor (~100s, contador `low_voltage_counter` SOLO con `!force` — el pre-check ya no lo pre-carga) → **`doDeepSleep`** (preflight + `RadioInterface::sleep()` + GPS + pantalla + System OFF) → ~1 mA. [Vivo] en [corte−100, corte) **opera** ~100s antes de dormir. [Listo] con V ≥ corte. [Boot] a los 2 min con causa RESETREAS. LED de estado apagado antes de System OFF (main-nrf52.cpp). Avisos = `ADC + CPU` (chip, reintento BUSY; sin INA/I2C).
