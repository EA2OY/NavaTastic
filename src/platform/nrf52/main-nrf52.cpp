...
void cpuDeepSleep(uint32_t msecToWake)
{
    ...
    if (msecToWake != portMAX_DELAY &&
        ...
        // NAVARICO (endurecimiento LPCOMP, 11/09): espera ACOTADA en vez de bucle infinito.
        // El READY del LPCOMP es de microsegundos en condiciones validas; si no llegara,
        // NUNCA se entra en System OFF sin fuente de despertar: se reintenta el re-armado
        // una vez y, si sigue sin estar listo, el nodo se reinicia solo (autonomia solar,
        // sin boton). En el caso normal (READY inmediato) el comportamiento no cambia.
