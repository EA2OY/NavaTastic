...
> ⚠️ **AVISO IMPORTANTE — V5.1 ESTÁ EN FASE ALPHA**:  
> La versión **NavaTastic V5.1** se encuentra en **fase ALPHA**, siendo probada en campo por el
> autor. Incluye funciones potentes y nuevas (el Botón del Pánico consolidable, la instalación
> automática, el reenvío configurable...) y además corrige un detalle importante: **el nombre
> que pones a tu nodo con la App oficial ya no se pierde al reiniciar** (se recuerda solo) y el
> nodo muestra un **contador de restablecimientos de fábrica** (`FR`) en `/nava status` para
> saber si un repetidor se ha restablecido sin que tú lo ordenaras. Aun así, **úsala con
> criterio y no la consideres "estable"** hasta que el autor confirme su comportamiento tras
> las pruebas reales.  
...
> ⚠️ **IMPORTANT NOTICE — V5.1 IS IN ALPHA PHASE**:  
> The **NavaTastic V5.1** version is being field-tested by the author. It includes powerful new
> features, and it also fixes an important detail: **the name you set with the official
> Meshtastic app is no longer lost on reboot** (the node remembers it), and the node shows a
> **factory-reset counter** (`FR`) in `/nava status` so you can tell whether a repeater reset
> itself without your command. Still, it has not passed final validation yet: **use it with
> judgment and do not consider it "stable"** until the author confirms its behavior after
> real-world testing.  
